$M"tpcc" /* the module file base name */
{: Turbo Pascal Command Line Complier.}
program -->MODULENAME<--;
{$APPTYPE CONSOLE}
...
const
  ...
  cProgTitle     = 'Turbo Pascal Command Line Complier';
  cTurboPascalFileExt = '.pas';
...
begin
  ...
  //Writeln('');
  Writeln('Usage: -->MODULENAME<-- filename['+cTurboPascalFileExt+'] [-ge]');
  Writeln('Options:');
  Writeln('-ge generate error detail list file if nay.');
  Writeln('Example: -->MODULENAME<-- Test'+cTurboPascalFileExt);
end;
  ...
  try
    ...
    try
      if ExtractFileExt(GFileName) = '' then 
        GFileName := GFileName + cTurboPascalFileExt;
