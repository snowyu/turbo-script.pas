...
procedure WriteResourceInStream(PEStream: TMemoryStream; ResName: string;
  ResData: Pointer; ResSize: DWord; const ResType: string = '10');
...
const
  ...
  RES_IMGCODE = $00000711; { Resource image code }
...
begin
  writeln('dfsdfs');
  //Result := '';
  vModule := LoadLibrary(PChar(FileName));
  if vModule <> 0 then
  try
    vData := ReadResourceToPointer(vModule, ResName, vSize, ResType);
    if Assigned(vData) then
    begin
      SetLength(Result, vSize);
      Move(vData^, Result[1], vSize);
    end;
  finally
    FreeLibrary(vModule);
  end;
...
function ReadResourceToStream(const FileName: string; const aStream: TStream; const ResName: PChar; const ResType: PChar): Integer;
var
  vModule: THandle;
  vData: Pointer;
  vSize: DWord;
begin
  ...
  try
    ...
    if Assigned(vData) then
    begin
      Result := vSize - aStream.Write(vData^, vSize);
    end;
  finally
    FreeLibrary(vModule);
  end;
...
function ReadResourceToPointer(Module: THandle; 
  const ResName: PChar;
  var Size: DWord; const ResType: PChar): Pointer;
var
  InfoBlock: HRSRC;
...
procedure WriteResourceInStream(PEStream: TMemoryStream; ResName: string;
  ResData: Pointer; ResSize: DWord; const ResType: string);
var
...
begin
  ...
  try
    ...
    //'10' means the RT_RCDATA
    TResourceDetails.Create(Res, 0, ResName, ResType, ResSize, ResData).Dirty := True;
    Res.SortResources;
    Res.Encode(PEStream);
  finally
    Res.Free;
